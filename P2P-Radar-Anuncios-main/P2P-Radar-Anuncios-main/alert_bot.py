...
"""
import os, sys, time, json, urllib.request, urllib.parse, logging, requests
...
TAKER_FEE     = 0.14  # $0.07 compra + $0.07 venta en USDT
...
def gh_save_file(path: str, content: str, sha: str = None, message: str = "bot: update history"):
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            logger.error(f"Error guardando {path}: {r.status_code} {r.text[:100]}")
            return False
    except Exception as e:
...
def update_history(history: dict, metodo: dict, buy_price: float, sell_price: float,
                   taker_buy: float, taker_sell: float) -> dict:
    key = metodo["id"]
    entry = {
        "ts":          datetime.now(timezone.utc).isoformat(),
        "buy_price":   round(buy_price,  2),   # precio maker compra
        "sell_price":  round(sell_price, 2),   # precio maker venta
        "taker_buy":   round(taker_buy,  2),   # mejor precio taker compra
        "taker_sell":  round(taker_sell, 2),   # mejor precio taker venta
        "spread_pct":  round((sell_price - buy_price) / buy_price * 100, 4) if buy_price > 0 else 0,
    }
    if key not in history:
        history[key] = {"nombre": metodo["human_name"], "fiat": metodo["fiat"], "registros": []}
...
def send_telegram(username: str, text: str) -> bool:
    ...
    try:
        ...
        if ok: logger.info(f"✅ Telegram enviado a @{username}")
        else:  logger.warning(f"⚠️ CallMeBot: {body[:100]}")
        return ok
    except Exception as e:
...
def build_maker_message(result, pct) -> str:
    ...
    return (
        ...
        f"🔴 {f'{buy.min_amount:,.0f}' if buy else '—'} – {f'{buy.max_amount:,.0f}' if buy else '—'} {result.fiat}\n"
        f"💳 {pays(buy)}\n{sep}\n"
        f"📕 VENDER (tab Vender)\n"
        f"👤 {sell.advertiser_name if sell else '—'}\n"
        f"💵 {fmt(result.sell_price_effective)} {result.fiat} / {result.asset}\n"
        f"🔴 {f'{sell.min_amount:,.0f}' if sell else '—'} – {f'{sell.max_amount:,.0f}' if sell else '—'} {result.fiat}\n"
        f"💳 {pays(sell)}"
    )
...
def main():
    ...
    logger.info(f"🔍 Consultando | Umbral: {UMBRAL_PCT}% | Fee COP: {MAKER_FEE_COP}%")
    ...
    for metodo in METODOS_COP:
        ...
        # Precios taker (top 1 de cada lado)
        taker_buy  = buy_ads[0]["price"]   # precio más bajo para comprar
        taker_sell = sell_ads[0]["price"]  # precio más alto para vender

        # ── Estrategia Maker-Maker (normal) ───────────────────────────────────
        result = analyze_opportunity(
            buy_ads=buy_ads, sell_ads=sell_ads,
            exchange="Binance", method_id=metodo["id"],
            method_human=nombre, method_mapped=metodo["payment_id"],
            fiat=fiat, asset=ASSET,
            fee_exchange_pct=MAKER_FEE_COP,
            fee_slippage_pct=0.0, fee_method_pct=0.0, fee_method_fixed=0.0,
            publication_mode="match_top",
            epsilon_fiat=0.0, epsilon_pct=0.0,
            exclude_ads=True, ad_threshold_pct=999.0,
            capital_target_min=CAPITAL_COP["target_min"],
            capital_target_max=CAPITAL_COP["target_max"],
            capital_simulado=CAPITAL_COP["simulado"],
            buffer_pct=1.0, min_net_profit_pct=0.0, allow_outside_target=True,
        )

        pct = result.net_profit_pct or 0
        logger.info(f"    Maker | Spread: {result.spread_gross_pct:.4f}% | Neto: {pct:.4f}%")

        if result.status == "OK" and pct >= UMBRAL_PCT:
            logger.info(f"    ✅ Oportunidad maker! Enviando...")
            if send_telegram(TG_USERNAME, build_maker_message(result, pct)):
                alertas_enviadas += 1
            time.sleep(2)

        # ── Estrategia Taker-Taker ────────────────────────────────────────────
        # taker_buy = precio al que compramos (tab Comprar, el más bajo)
        # taker_sell = precio al que vendemos (tab Vender, el más alto)
        if taker_sell > taker_buy:
            cantidad_usdt  = CAPITAL_COP["simulado"] / taker_buy
            ganancia_bruta = (taker_sell - taker_buy) * cantidad_usdt
            ganancia_cop   = ganancia_bruta - (TAKER_FEE * taker_sell)  # descuento fee en COP
            ganancia_pct   = (taker_sell - taker_buy) / taker_buy * 100

            logger.info(f"    Taker | Compra: {taker_buy:,.2f} | Venta: {taker_sell:,.2f} | Ganancia: {ganancia_cop:,.0f} COP ({ganancia_pct:.3f}%)")

            if ganancia_pct >= UMBRAL_PCT:
                logger.info(f"    🟠 Oportunidad taker! Enviando...")
                msg = build_taker_message(metodo, taker_buy, taker_sell, ganancia_cop, cantidad_usdt)
        ...
        history = update_history(history, metodo, buy_price, sell_price, taker_buy, taker_sell)
